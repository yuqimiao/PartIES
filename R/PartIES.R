...
# * weight_def: different empirical weight definition, option 1 or 2
# Output:
...
parties = function(kernel_list,
  ...
  if(use_kernel == T){
    kernel_distance_list = lapply(kernel_list, function(kernel) 2-2*(kernel))
  }
  # initialization ----
  # par initial
  ...
  if(update_neig){
    W = Reduce("+",lapply(F0, function(f) f %*% t(f)))/S
    print(dim(W))
    c = estimateNumberOfClustersGivenGraph(W = W, NUMC = numc)[[1]]
    print(c)
  }
